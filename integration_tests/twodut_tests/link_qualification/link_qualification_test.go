...
func TestLinkQualification(t *testing.T) {
	...
	configureInterfaces(t, dut1, dut2, port1.Name(), port2.Name())
	...
	createGeneratorQualification(t, gnoiClient1, generatorID, port1.Name(), timing, genCapabilities)

	// Create reflector qualification on dut2
	createReflectorQualification(t, gnoiClient2, reflectorID, port2.Name(), timing, refCapabilities)

	// Monitor both qualifications until completion
	monitorQualifications(t, gnoiClient1, gnoiClient2, generatorID, reflectorID, dut1, dut2, port1.Name(), port2.Name())
...
func configureInterfaces(t *testing.T, dut1, dut2 *ondatra.DUTDevice, intf1, intf2 string) {
	t.Helper()
	duts := []*ondatra.DUTDevice{dut1, dut2}
	interf := []string{intf1, intf2}

	for i, dut := range duts {
		interfaceName := interf[i]
		intf := &oc.Interface{
		...
		gnmi.Replace(t, dut, ocpath.Root().Interface(interfaceName).Config(), intf)

		// Wait for interface to come up
		gnmi.Await(t, dut, ocpath.Root().Interface(interfaceName).OperStatus().State(),
...
func createGeneratorQualification(t *testing.T, client gnoigo.Clients, id string, interfaceName string, timing *LinkQualificationTiming, caps *plqpb.CapabilitiesResponse) {
	t.Helper()
...
func createReflectorQualification(t *testing.T, client gnoigo.Clients, id string, interfaceName string, timing *LinkQualificationTiming, caps *plqpb.CapabilitiesResponse) {
	t.Helper()
...
func monitorQualifications(t *testing.T, client1, client2 gnoigo.Clients, genID, refID string, dut1, dut2 *ondatra.DUTDevice, intf1, intf2 string) {
	t.Helper()
	...
	for time.Since(startTime) < waitTimeout {
		...
		if genResult.GetState() == plqpb.QualificationState_QUALIFICATION_STATE_RUNNING {
			checkInterfaceStatus(t, dut1, intf1, "generator")
		}
		if refResult.GetState() == plqpb.QualificationState_QUALIFICATION_STATE_RUNNING {
			checkInterfaceStatus(t, dut2, intf2, "reflector")
		}
