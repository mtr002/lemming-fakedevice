...
const (
	...
	defaultRestartTime = 2 * time.Second
	defaultProcessPID  = 1008
	defaultMaxPID      = 65535

	// LinkQualification defaults
	defaultUpdateTicker = 1 * time.Second

	defaultPreSyncDelay = 0 * time.Second
	defaultSetupTime    = 1 * time.Second
	defaultTestDuration = 5 * time.Second
	defaultTeardownTime = 1 * time.Second

	defaultPacketRate = 138888
	defaultPacketSize = 8184

	// System defaults
	defaultCPUUser       = 1000000
	defaultCPUSys        = 500000
	defaultCPUUtil       = 1
	defaultMemUsage      = 10485760
	defaultMemUtil       = 2
	defaultInterfaceSize = 9000
)
...
func KillProcess(ctx context.Context, c *ygnmi.Client, pid uint32, processName string, signal spb.KillProcessRequest_Signal, restart bool, cfg *configpb.LemmingConfig) error {
	...
	if restart {
		go func() {
			...
			newProcess := createDefaultProcess(processName, newPID, uint64(restartTime))
...
// createDefaultProcess creates a process with default resource usage values
func createDefaultProcess(name string, pid uint32, startTime uint64) *oc.System_Process {
	return &oc.System_Process{
		Name:              ygot.String(name),
		Pid:               ygot.Uint64(uint64(pid)),
		StartTime:         ygot.Uint64(startTime),
		CpuUsageUser:      ygot.Uint64(defaultCPUUser),
		CpuUsageSystem:    ygot.Uint64(defaultCPUSys),
		CpuUtilization:    ygot.Uint8(defaultCPUUtil),
		MemoryUsage:       ygot.Uint64(defaultMemUsage),
		MemoryUtilization: ygot.Uint8(defaultMemUtil),
	}
}
...
func PingSimulation(ctx context.Context, destination string, count int32, interval time.Duration, wait time.Duration, size uint32, responseChan chan<- *PingPacketResult, cfg *configpb.LemmingConfig) error {
	...
	for packetCount < maxPackets {
		...
		case <-ticker.C:
			...
			networkLatency := getLatency(cfg)
			packetLost := packetLossRate > 0 && randGen.Float32() < packetLossRate
...
func NewChassisComponentsTask(cfg *configpb.LemmingConfig) *reconciler.BuiltReconciler {
	rec := reconciler.NewBuilder("chassis components").
		WithStart(func(ctx context.Context, c *ygnmi.Client) error {
			...
			linecardPrefix := cfg.GetComponents().GetLinecardPrefix()
			linecardCount := cfg.GetComponents().GetLinecard().GetCount()
			linecardStart := cfg.GetComponents().GetLinecard().GetStartIndex()
			linecardStep := cfg.GetComponents().GetLinecard().GetStep()

			for i := int32(0); i < linecardCount; i++ {
				index := linecardStart + (i * linecardStep)
				componentName := fmt.Sprintf("%s%d", linecardPrefix, index)
				component := &oc.Component{
					Name:             ygot.String(componentName),
			...
			fabricPrefix := cfg.GetComponents().GetFabricPrefix()
			fabricCount := cfg.GetComponents().GetFabric().GetCount()
			fabricStart := cfg.GetComponents().GetFabric().GetStartIndex()
			fabricStep := cfg.GetComponents().GetFabric().GetStep()

			for i := int32(0); i < fabricCount; i++ {
				index := fabricStart + (i * fabricStep)
				componentName := fmt.Sprintf("%s%d", fabricPrefix, index)
				component := &oc.Component{
					Name:             ygot.String(componentName),
...
func NewProcessMonitoringTask(cfg *configpb.LemmingConfig) *reconciler.BuiltReconciler {
	rec := reconciler.NewBuilder("process monitoring").
		WithStart(func(ctx context.Context, c *ygnmi.Client) error {
			...
			// Get processes from configuration (always provided by config loader defaults)
	...
	rec := reconciler.NewBuilder("interface initialization").
		WithStart(func(ctx context.Context, c *ygnmi.Client) error {
			...
			baseInterfaces := []struct {
				...
				{"eth1", "Standard Ethernet interface", 2},
				{"eth2", "Line card port 1", 3},
				{"eth3", "Line card port 2", 4},
				{"eth4", "Line card port 3", 5},
				{"eth5", "High-speed port 1", 6},
				{"Ethernet3/2", "High-speed port 2", 7},
				{"Ethernet4/1", "Fabric port 1", 8},
				{"Ethernet4/2", "Fabric port 2", 9},
			}
...
func generateNewPID(ctx context.Context, c *ygnmi.Client, excludePID uint32) (uint32, error) {
	...
	// Use a simple PID range starting from 1008 (after the default processes)
	for pid := uint32(defaultProcessPID); pid <= defaultMaxPID; pid++ {
		if !used[pid] {
			return pid, nil
		}
	}
	return 0, fmt.Errorf("no PID available in range 1008-65535")
}

// getLatency returns a simulated latency based on configured values.
func getLatency(cfg *configpb.LemmingConfig) time.Duration {
	if cfg != nil && cfg.GetNetworkSim() != nil {
		base := cfg.GetNetworkSim().GetBaseLatencyMs()
		jitter := cfg.GetNetworkSim().GetLatencyJitterMs()
		if base > 0 {
			latency := base
			if jitter > 0 {
				randLock.Lock()
...
func RunPacketLinkQualification(
	...
	if err := executeQualificationStateMachine(ctx, c, interfaceName, result, packetConfig, timing, updateChan, cfg); err != nil {
		result.mu.Lock()
...
func extractQualificationTiming(config *plqpb.QualificationConfiguration) *QualificationTiming {
	timing := &QualificationTiming{
		PreSyncDelay:     defaultPreSyncDelay,
		SetupDuration:    defaultSetupTime,
		TestDuration:     defaultTestDuration,
		TeardownDuration: defaultTeardownTime,
	}
...
func executeQualificationStateMachine(
	...
	if err := executeRunningPhase(ctx, result, packetConfig, timing, updateChan, cfg); err != nil {
