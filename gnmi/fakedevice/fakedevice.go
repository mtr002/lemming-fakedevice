...
func KillProcess(ctx context.Context, c *ygnmi.Client, pid uint32, processName string, signal interface{}, restart bool) error {
	...
	signalType, ok := signal.(spb.KillProcessRequest_Signal)
	if !ok {
		return fmt.Errorf("invalid signal type: %T", signal)
	}
	// HUP signal - reload configuration
	if signalType == spb.KillProcessRequest_SIGNAL_HUP {
...
func generateNewPID(ctx context.Context, c *ygnmi.Client) (uint32, error) {
	...
	for pid := uint32(1005); pid <= 1100; pid++ {
		if !used[pid] {
			return pid, nil
		}
	}
