	...
	tests := map[string]struct {
		...
		"defaults-all-default-values": {
			fn: func(t *testing.T, s *system, ctx context.Context) {
				...
				err := s.Ping(&spb.PingRequest{Destination: "192.168.1.1"}, stream)
				if err != nil {
					t.Fatalf("Unexpected error: %v", err)
				}

				expectedResponses := defaultPingCount + 1 // individual + summary
				...
				if summary.Sent != defaultPingCount {
					t.Errorf("Summary: expected sent %d, got %d", defaultPingCount, summary.Sent)
				}
				if summary.Received == 0 {
					t.Error("Summary: expected some received packets")
				}
			},
		},
