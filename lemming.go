...
func New(targetName, zapiURL string, opts ...Option) (*Device, error) {
	...
	// Log fault configuration status
	if lemmingConfig.GetFaultConfig() != nil && len(lemmingConfig.GetFaultConfig().GetGnoiFaults()) > 0 {
		log.Infof("Loaded %d gNOI fault configurations", len(lemmingConfig.GetFaultConfig().GetGnoiFaults()))
		for _, faultConfig := range lemmingConfig.GetFaultConfig().GetGnoiFaults() {
			log.Infof("Fault config for RPC %s: %d faults", faultConfig.GetRpcMethod(), len(faultConfig.GetFaults()))
		}
	} else {
		log.Info("No gNOI fault configurations loaded")
	}
