...
func TestDetermineConfigPath(t *testing.T) {
	tests := []struct {
		...
		{
			name:      "env value is known vendor cisco",
			flagValue: "",
			envValue:  "cisco",
			expected:  "embedded:cisco_default.textproto",
			wantError: false,
		},
		{
	...
	tests := []struct {
		...
		},
		{
			...
			config: &configpb.LemmingConfig{
				...
				NetworkSim: &configpb.NetworkSimConfig{
